...
from HK_SilkSong_env.HK_Silksong_env import HK_Silksong_Env
import train


def build_hk_env(image_size, seed, env_name: str):
    """构建 HollowKnight 环境，与 train_HK.py 保持一致"""
    # 根据 env_name 选择具体环境
    if env_name == "HollowKnight_Silksong":
        env = HK_Silksong_Env()
    else:
        env = HKEnv()
...
def eval_episodes(num_episode, image_size, world_model: WorldModel, 
                  agent: agents.ActorCriticAgent, env_name: str, seed=0):
    """评估 HollowKnight 环境"""
    world_model.eval()
    agent.eval()
    
    env = build_hk_env(image_size, seed, env_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-env_name", type=str, default="HollowKnight", help="Environment name")
    ...
    if env_name == "HollowKnight_Silksong":
        action_dim = 256           # MultiBinary(8) -> Discrete(256)
    elif env_name == "HollowKnight":
        action_dim = 128           # MultiBinary(7) -> Discrete(128)
